...
#+AUTHOR: Derek Taylor (DT) (orig author)
#+DESCRIPTION: Emacs config
#+STARTUP: showeverything

# Modification history:
# 12.Jan.2023    Initial mods to appearance:
#                theme, typefaces, font size, etc.
# 14.Jan.2023    Installed and configured Denote package.
...
* ABOUT THIS CONFIG
This is based on the personal Emacs config of Derek Taylor
[[https://gitlab.com/dwt1/dotfiles/-/blob/master/.config/doom/config.org]]. Doom Emacs
is a distribution of Emacs that uses the "evil" keybindings (Vim keybindings)
and includes a number of nice extensions and a bit of configuration out of the
box.
...
* DENOTE
From the [[https://protesilaos.com/emacs/denote#h:a09b70a2-ae0b-4855-ac14-1dddfc8e3241][Denote]] docs: Denote aims to be a simple-to-use, focused-in-scope, and effective note-taking tool for Emacs.
...
* ORG MODE
Most of this block is wrapped in (after! org).  Without this, the settings might be evaluated too early, which will result in their being overwritten by Doom's defaults. Also enabled are: org-journal, org-superstar, and org-roam by adding (+journal +pretty +roam2) to the org section of the Doom Emacs init.el.

=NOTE=: Location of the Org directory and Roam directory in $HOME/org.
...
** Org-export
We need ox-man for "Org eXporting" to manpage format, ox-gemini for exporting to gemtext (for the gemini protocol), and ox-pandoc for exporting with pandoc.
...
* WINNER MODE
Winner mode has been included with GNU Emacs since version 20.  This is a global minor mode and, when activated, it allows you to “undo” (and “redo”) changes in the window configuration with the key commands 'SPC w <left>' and 'SPC w <right>'.
